...
import { toolNavSections, tutorialNavSections, learningPaths } from "@/lib/navigation";
...
function PathsDropdown({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  return (
    <div className="relative">
      <button
        className={`flex items-center gap-1 rounded-md px-2 py-1.5 text-sm font-medium transition-colors ${
      ...
      {isOpen && (
        <div className="absolute left-0 top-full z-50 mt-1 w-80 rounded-xl border border-gray-200 bg-white p-3 shadow-xl dark:border-gray-700 dark:bg-gray-900">
          <ul className="space-y-1">
            {learningPaths.map((path) => (
              <li key={path.slug}>
                <Link
                  ...
                  onClick={onClose}
                  className="flex items-center gap-3 rounded-lg px-2 py-2 text-sm transition-colors hover:bg-gray-100 dark:hover:bg-gray-800"
                >
                  <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-blue-50 text-base dark:bg-blue-950">
                    {path.icon}
                  </span>
                  <div className="min-w-0">
                    <p className="font-medium text-gray-900 dark:text-white">{path.title}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">~{path.estimatedHours}h · {path.difficulty}</p>
                  </div>
                </Link>
              </li>
            ))}
...
export default function GlobalNavbar() {
  ...
  const [openMenu, setOpenMenu] = useState<"tutorials" | "tools" | "paths" | null>(null);
  ...
  function toggleMenu(menu: "tutorials" | "tools" | "paths") {
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-2.5">
          ...
          <div className="flex items-center gap-1">
            ...
            <div className="hidden items-center gap-0.5 lg:flex">
              ...
              />
              <PathsDropdown
                isOpen={openMenu === "paths"}
                onClose={() => toggleMenu("paths")}
              />
            </div>
          </div>
